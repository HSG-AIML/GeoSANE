...
class Bezier(Module):
    ...
    def forward(self, t):
        return (
            self.binom * torch.pow(t, self.range) * torch.pow((1.0 - t), self.rev_range)
        )
...
class LayerNorm(CurveModule):
    ...
    def __init__(
        self, normalized_shape, fix_points, eps=1e-5, momentum=0.1, affine=True
    ):
    ...
    def forward(self, input, coeffs_t):
        weight_t, bias_t = self.compute_weights_t(coeffs_t)
        return F.layer_norm(input, self.normalized_shape, weight_t, bias_t, self.eps)
...
class CurveNet(Module):
    def __init__(
        ...
        self.net = self.architecture(
            num_classes, fix_points=self.fix_points, **architecture_kwargs
        )
        self.curve_modules = []
    ...
    def import_base_parameters(self, base_model, index):
        ...
        for param_name, param_obj in param_dict.items():
            if "base_param" not in param_obj:
                ...
                continue
            # print(param_name, param_obj['param'].data.shape, param_obj['base_param'].data.shape)
            param_obj["param"].data.copy_(param_obj["base_param"].data)

    def import_base_buffers(self, base_model):
        for buffer, base_buffer in zip(
            self.net._all_buffers(), base_model._all_buffers()
        ):
