...
from .def_resnet_width import ResNet_width
...
# define FNNmodule
# ##############################################################################
class NNmodule_width(nn.Module):
    def __init__(self, config, cuda=False, seed=42, verbosity=0):
        super(NNmodule_width, self).__init__()
        ...
        if config.get("model::data_parallel", False):
        ...
        self.accumulate_gradients_steps = config.get(
            "training::accumulate_gradients_steps", 1
        )
    ...
    def train_step(self, input, target, idx):
        ...
        # scale loss with gradient accumulation steps
        loss = loss / self.accumulate_gradients_steps
        # prop loss backwards to
        loss.backward()
        # accumulate gradients:
        # # if idx+1 is multiple of accumulate_gradients_steps, update parameters
        if (idx + 1) % self.accumulate_gradients_steps == 0:
            # update parameters
            self.optimizer.step()
    ...
    def train_epoch(self, trainloader, epoch, idx_out=10):
        ...
        for idx, data in enumerate(trainloader):
            ...
            loss, correct = self.train_step(input, target, idx)
