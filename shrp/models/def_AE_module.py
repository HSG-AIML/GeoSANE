...
import tqdm
from shrp.datasets.augmentations import IdentityTransform
...
class AEModule(nn.Module):
    def __init__(self, config):
        ...
        self.use_layer_types = config.get("ae:use_layer_embs", False)
        self.use_layer_types_enc_only = config.get("ae:use_layer_embs_enc_only", False)
    ...
    def set_transforms(
        ...
        if transforms_train is not None:
            ...
            self.transforms_train = IdentityTransform()
        if transforms_test is not None:
            self.transforms_test = transforms_test
        else:
            self.transforms_test = IdentityTransform()
        if transforms_downstream is not None:
            self.transforms_downstream = transforms_downstream
        else:
            self.transforms_downstream = IdentityTransform()

    def forward(self, x, p, l=None):
        if not self.use_layer_types:
            l = None
        # pass forward call through to model
        logging.debug(f"x.shape: {x.shape}")
        z = self.forward_encoder(x, p, l=l)
        logging.debug(f"z.shape: {z.shape}")
        zp = self.model.projection_head(z)
        logging.debug(f"zp.shape: {zp.shape}")
        y = self.forward_decoder(z, p, l=l)
        logging.debug(f"y.shape: {y.shape}")
        return z, y, zp

    def forward_encoder(self, x, p, mask=None, l=None):
        # normalize input features
        z = self.model.forward_encoder(x, p, mask=mask, layer_type=l)
        return z

    def forward_decoder(self, z, p, mask=None, l=None):
        y = self.model.forward_decoder(z, p, mask=mask, layer_type=l)
        return y
    ...
    def set_scheduler(self, config):
        ...
        #     )
            ...
            total_steps = (
                ...
                * config.get("training::steps_per_epoch", 1000)
    ...
    # ##########################
    def train_step(self, x_i, m_i, p_i, x_j, m_j, p_j, l_i=None, l_j=None, idx=0):
        """
        ...
        Args:
            ...
            l_i (torch.Tensor): batch of layer types view 1
            l_j (torch.Tensor): batch of layer types view 2
        Returns:
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            if l_i is not None:
                z_i, y_i, zp_i = self.forward(x_i, p_i, l_i)
                z_j, y_j, zp_j = self.forward(x_j, p_j, l_j)
            else:
            ...
            loss = perf["loss/loss"]
            # rescale loss with accumulation steps
            loss = loss / self.accumulation_steps
            # technically, there'd need to be a scaler for each loss individually.
            self.scaler.scale(loss).backward()
        # if accumulation steps are reached, update parameters
        if (idx + 1) % self.accumulation_steps == 0:
            ...
            #     self.scheduler.step()
        # return perf
        perf["loss/loss"] = loss*self.accumulation_steps
        return perf
    ...
    def train_epoch(self, trainloader, epoch=0, show_progress=False):
        ...
        for idx, data in tqdm.tqdm(
            ...
            if self.use_layer_types:
                x_i, m_i, p_i, l_i, x_j, m_j, p_j, l_j = self.transforms_train(*data)
            else:
            ...
            p_i = p_i.to(self.device)
            p_j = p_j.to(self.device)
            # ### end hotfix
            # take train step
            perf = self.train_step(x_i, m_i, p_i, x_j, m_j, p_j, l_i, l_j, idx=idx)
            ...
            n_data += x_i.shape[0]
            # hotfix
            del x_i, m_i, p_i, x_j, m_j, p_j, l_i, l_j
    ...
    # test batch
    def test_step(self, x_i, m_i, p_i, x_j, m_j, p_j, l_i=None, l_j=None):
        """
        #TODO
        """
        with torch.no_grad():
            with torch.cuda.amp.autocast(enabled=True):
                # forward pass with both views
                # print(x_i)
                # print(torch.isnan(x_i).sum().item(), torch.isnan(p_i).sum().item())
                # print(torch.isnan(x_j).sum().item(), torch.isnan(p_j).sum().item())
                if l_i is not None:
                    z_i, y_i, zp_i = self.forward(x_i, p_i, l_i)
                    z_j, y_j, zp_j = self.forward(x_j, p_j, l_j)
                else:
                    ...
                    z_j, y_j, zp_j = self.forward(x_j, p_j)
                # print(
                #     torch.isnan(z_i).sum().item(),
                #     torch.isnan(y_i).sum().item(),
                #     torch.isnan(zp_i).sum().item()
                # )
                # print(
                #     torch.isnan(z_j).sum().item(),
                #     torch.isnan(y_j).sum().item(),
                #     torch.isnan(zp_j).sum().item()
                # )
                # cat y_i, y_j and x_i, x_j, and m_i, m_j
                x = torch.cat([x_i, x_j], dim=0)
                y = torch.cat([y_i, y_j], dim=0)
    ...
    # test epoch
    def test_epoch(self, testloader, epoch=0, show_progress=False, debug=False):
        ...
        for idx, data in tqdm.tqdm(
            ...
            # pass through transforms (if any)
            if self.use_layer_types:
                x_i, m_i, p_i, l_i, x_j, m_j, p_j, l_j = self.transforms_test(*data)
            else:
            ...
            ###
            # compute loss
            perf = self.test_step(x_i, m_i, p_i, x_j, m_j, p_j, l_i, l_j)
            # scale loss with batchsize (get's normalized later)
            if not debug:
                for key in perf.keys():
                    if key not in perf_out:
                ...
                for key in perf.keys():
                    if key not in perf_out:
                        perf_out[key] = [perf[key].item()]
                    else:
                        perf_out[key].append(perf[key].item())
